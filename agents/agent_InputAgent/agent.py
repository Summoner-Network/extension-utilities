...
async def send_handler() -> str:
    global tracker
    no_test = all(tracker[key] == 0 for key in tracker.keys())
    strip = ""
    if no_test:
    ...
    # Collect active test counters
    tested_keys = [key for key, key_count in tracker.items() if key_count != 0]

    # Publish the test to the server
    if strip in tracker.keys() or tested_keys:
        keys = tested_keys + [strip] if strip in tracker.keys() else tested_keys
        for key in keys:
            output = test_inputs[key][tracker[key] % len(test_inputs[key])]
            try:
                await aprint(output)
                return output
                ...
                tracker[key] += 1
                tracker[key] = tracker[key] % len(test_inputs[key])
